...
describe('WorkspaceService', () => {
    ...
    describe('updateWorkspaceNodeCount', () => {
        it('should update just the nodeCount and updatedAt on the workspace document', async () => {
            ...
            // Requires mockUpdateDoc to track this, adding it inline via vitest spy on firestore
    ...
    describe('loadWorkspace', () => {
        it('should load workspace from Firestore', async () => {
            ...
            expect(result?.nodeCount).toBe(0); // Default gracefully handles undefined
            expect(mockGetCountFromServer).not.toHaveBeenCalled(); // nodeCount implicitly backfilled below
        });
    ...
    describe('loadUserWorkspaces', () => {
        ...
        it('should backfill nodeCount using getCountFromServer if missing for any workspace in list', async () => {
            ...
            // Mock one legacy workspace without nodeCount, one regular workspace with nodeCount
            mockGetDocs.mockResolvedValue({
                docs: [
                    {
                        ref: { id: 'ref-1' },
            ...
            expect(result).toHaveLength(2);
            expect(result[0]!.nodeCount).toBe(5); // Backfilled
            expect(result[1]!.nodeCount).toBe(10); // Original
            expect(mockGetCountFromServer).toHaveBeenCalledTimes(1); // Only called for the legacy one
            expect(mockSetDoc).toHaveBeenCalledWith(
                { id: 'ref-1' },
                { nodeCount: 5 },
                { merge: true }
    ...
    describe('createNewWorkspace', () => {
        it('should create a new workspace with generated ID', async () => {
            ...
            expect(result.nodeCount).toBe(0); // Ensures new workspaces init with 0
        });
