...
describe('Grid Layout Service (Masonry)', () => {
    ...
    describe('Pinned Node Exclusion', () => {
        describe('arrangeMasonry with pinned nodes', () => {
            ...
            it('should arrange only unpinned nodes in the grid', () => {
                ...
                const pinned = arranged.find(n => n.id === 'pinned1');
                expect(pinned!.position).toEqual({ x: 999, y: 999 });

                const free1 = arranged.find(n => n.id === 'free1');
                const free2 = arranged.find(n => n.id === 'free2');
                const free3 = arranged.find(n => n.id === 'free3');

                expect(free1!.position.x).toBe(32);
                expect(free1!.position.y).toBe(32);
                expect(free2!.position.x).toBe(352);
                expect(free2!.position.y).toBe(32);
                expect(free3!.position.x).toBe(672);
                expect(free3!.position.y).toBe(32);
            });
