...
export const TransformMenu = React.memo(({
    onTransform, isOpen, onToggle, onClose, onRegenerate,
    disabled = false, isTransforming = false, tooltipPlacement,
}: TransformMenuProps) => {
    ...
    const handleToggle = useCallback(() => {
        if (!disabled && !isTransforming) onToggle();
    ...
    const buttonLabel = isTransforming ? strings.ideaCard.transforming : strings.ideaCard.transform;
        ...
        <div className={styles.dropdownMenu} role="menu"
            ...
            {TRANSFORM_OPTIONS.map(({ type, labelKey }) => (
                <button key={type} className={styles.menuItem}
                    onClick={() => handleSelect(type)} role="menuitem">
                    {strings.transformations[labelKey]}
                </button>
            ))}
            {onRegenerate && (
                <button className={styles.menuItem} onClick={handleRegenerate} role="menuitem">
    ...
    return (
        <div className={styles.transformMenuWrapper}>
            <button ref={buttonRef} className={styles.transformButton}
                onClick={handleToggle} disabled={disabled || isTransforming}
                aria-label={buttonLabel} aria-expanded={isOpen} aria-haspopup="menu">
                <span className={styles.icon}>{isTransforming ? '⏳' : '✨'}</span>
            </button>
            <PortalTooltip text={buttonLabel} targetRef={buttonRef}
